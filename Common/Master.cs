...
namespace Common
{
    public class Master
    {
        ...
        {
            //----UInt16 是两个字节
            //----UInt8 是1个字节  是8位2进制  0x开头表示 16进制 
            //----swap 是交换高低位，
        ...
        {
            //-------------fctReadInputRegister 代表4 代表读取输入寄存器
            //-------------unit 代表第几个通道
            //-------------后面的id 标识符

            //--------id 是客户端标识符
            //--------unit 是从站地址
            //--------startaddress 是寄存器起始地址
            //--------length 是寄存器的个数
            //--------function 是功能码
        ...
        public void WriteMultipleRegister(ushort id, byte unit, ushort startAddress, byte[] values)
        {
            ...
            byte[] data;
            //-----------创建
            //-----------fctWriteMultipleRegister 为数值16；写多个保持继电器
        ...
        public void WriteMultipleRegister(ushort id, byte unit, ushort startAddress, byte[] values, ref byte[] result)
        {
            ...
            if (numBytes % 2 > 0) numBytes++;
            byte[] data;
            //-------fctWriteMultipleRegister 写多个保持继电器
            data = CreateWriteHeader(id, unit, startAddress, Convert.ToUInt16(numBytes / 2), Convert.ToUInt16(numBytes + 2), fctWriteMultipleRegister);
            Array.Copy(values, 0, data, 13, values.Length);
            //-------建立新的数组。。。
            result = WriteSyncData(data, id);
        ...
        private byte[] CreateReadHeader(ushort id, byte unit, ushort startAddress, ushort length, byte function)
        {
            ...
            byte[] data = new byte[12];  //---一个字有2个字节，1个字节8位(00-FF),2个字节16位(0000-FFFF)

            byte[] _id = BitConverter.GetBytes((short)id);
            data[0] = _id[1];			    // Slave id high byte  
            data[1] = _id[0];				// Slave id low byte  //----客户端标识符
            data[5] = 6;					// Message size
            data[6] = unit;					// Slave address  //---从站地址
            data[7] = function;				// Function code  //----代表功能  :初始化读取码 4-(读取输入寄存器)
            byte[] _adr = BitConverter.GetBytes((short)IPAddress.HostToNetworkOrder((short)startAddress));
            data[8] = _adr[0];				// Start address   //----单元0的起始地址0 //----单元1的起始地址76
        ...
        private byte[] CreateWriteHeader(ushort id, byte unit, ushort startAddress, ushort numData, ushort numBytes, byte function)
        {
            ...
            byte[] _id = BitConverter.GetBytes((short)id);
            data[0] = _id[1];				// Slave id high byte  //id 为 客户端的标识符
            ...
            data[9] = _adr[1];				// Start address
            if (function >= fctWriteMultipleCoils)  //---------如果执行的函数数值大于15.。。。。。
            {
                byte[] _cnt = BitConverter.GetBytes((short)IPAddress.HostToNetworkOrder((short)numData));
                data[10] = _cnt[0];			// Number 寄存器的个数
                data[11] = _cnt[1];			// Number 寄存器的个数
                data[12] = (byte)(numBytes - 2);  //----需要写入的字节的数量
            }
            return data;
        }
        ...
        private byte[] CreateReadWriteHeader(ushort id, byte unit, ushort startReadAddress, ushort numRead, ushort startWriteAddress, ushort numWrite)
        {
            ...
            data[0] = _id[1];						// Slave id high byte  ---事务处理标识符低位  客服机发起，服务器复制
            //--------------
            data[1] = _id[0];						// Slave id low byte  ---事务处理标识符高位  客服机发起，服务器复制
            //--------------
            byte[] _size = BitConverter.GetBytes((short)IPAddress.HostToNetworkOrder((short)(11 + numWrite * 2)));
            data[4] = _size[0];						// Complete message size in bytes  //----信息长度低位  
            data[5] = _size[1];						// Complete message size in bytes  //----信息长度高位
            data[6] = unit;							// Slave address  //----------单元标识符  客服机发起，服务器复制
            data[7] = fctReadWriteMultipleRegister;	// Function code   //------------功能符号
            byte[] _adr_read = BitConverter.GetBytes((short)IPAddress.HostToNetworkOrder((short)startReadAddress));
            data[8] = _adr_read[0];					// Start read address  //----开始读的起始地址
            data[9] = _adr_read[1];					// Start read address  //----起始地址
            byte[] _cnt_read = BitConverter.GetBytes((short)IPAddress.HostToNetworkOrder((short)numRead));
            data[10] = _cnt_read[0];				// Number of bytes to read  //----读取寄存器数量
            data[11] = _cnt_read[1];				// Number of bytes to read  //----读取寄存器数量
            byte[] _adr_write = BitConverter.GetBytes((short)IPAddress.HostToNetworkOrder((short)startWriteAddress));
            data[12] = _adr_write[0];				// Start write address  //----开始写的起始地址
            data[13] = _adr_write[1];				// Start write address  //----开始写的起始地址
            byte[] _cnt_write = BitConverter.GetBytes((short)IPAddress.HostToNetworkOrder((short)numWrite));
            data[14] = _cnt_write[0];				// Number of bytes to write   //----写的寄存器的数量
            data[15] = _cnt_write[1];				// Number of bytes to write   //----写的寄存器的数量
        ...
        private void WriteAsyncData(byte[] write_data, ushort id)
        {
            ...
            if ((tcpAsyCl != null) && (tcpAsyCl.Connected))
            {
                try
                {
                    ...
                    //----------等待接收数据OnReceive，如果有数据接收过来的话。

                    //--------客户端发送数据给服务器

                    //---情况1：要求读取寄存器0-11的数值。这些返回值赋值给了tcpAsyClBuffer。
        ...
        private void OnReceive(System.IAsyncResult result)
        {
            ...
            //-------第一步：将只读字节范围转换为 16 位无符号整数
            //-------第二步：将16位无符号整数，高低位交换。
            //-------Uint16 等于2个字节(bytes)，1个byte等于8bits;
            ...
            {
                //------功能吗 读取小于5，写入>=5
                //--------复制的长度为：tcpAsyClBuffer[8]
                data = new byte[tcpAsyClBuffer[8]];  //----读取时返回的寄存器个数
            ...
            // ------------------------------------------------------------
            // Response data is regular data----返回的数据如果正常的话
